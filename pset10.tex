...
\newpage    
  ...
  \part If $A$ and $B$ are sets with $|A| = |B|$, then $|\mcc{P}(A)| = |\mcc{P}(B)|$.
    \begin{solution}
      We provide a direct proof, using the results that two sets have the same cardinality iff a bijection exists between them.

      \begin{proof} $(|A| = |B|)\implies (\mcc{P}(A)| = |\mcc{P}(B)|)$

        Consider a bijection, $f:A\to B$.\\
        Then, for a subset $X$ of $A$, we have that $f(X)$ is a subset of $B$.\\
        Consider a function, $g:\mcc{P}(A) \to \mcc{P}(B)$ such that $g(X) = f(X)$.\\
        Because $f$ is a bijection, so is $g$.\\
